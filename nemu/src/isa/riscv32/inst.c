...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  slli   , I, imm &= 0xfffff01f,Print_DBG_Message("slli"),R(rd) = src1 << imm);
  
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", \
  srli   , I, imm &= 0xfffff01f,Print_DBG_Message("srli"),R(rd) = src1 >> imm);
  
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", \
  srai   , I, imm &= 0xfffff01f,Print_DBG_Message("srai"),R(rd) = (sword_t)src1 >> imm);
