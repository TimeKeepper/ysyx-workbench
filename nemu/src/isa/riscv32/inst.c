...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  csrrw  , I, Print_DBG_Message("csrrw"),     R(rd) = SR(imm), SR(imm) = src1, printf("nemu changing sr[%d] to %x\n", imm, src1));
  
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", \
  csrrs  , I, Print_DBG_Message("csrrs"),     R(rd) = SR(imm), SR(imm) |= src1, printf("nemu setting sr[%d] to %x\n", imm, src1));
