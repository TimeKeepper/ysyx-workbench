...
ysyx_macro::mod_flat!(simulator, cmd);
...
impl Monitor {
    ...
    pub fn main_loop(&mut self) {
        ...
        while self.state != Monitor_state::QUIT && self.state != Monitor_state::ABORT {
    ...
    fn deal_result(&mut self, result: Result<simOk, simErr>) {
        match result {
            ...
            Err(err) => match err {
                ...
                simErr::NoMatchingMemoryByAddress { addr } => self
                    .msgr
                    .error(format!("No matching memory {}", addr).as_str()),
                simErr::NoMatchingMemoryByName { name } => self
                    .msgr
                    .error(format!("No matching memory {}", name).as_str()),
                simErr::InstrctionDecodeFailed { inst } => self.msgr.error(
                    format!(
                ...
                simErr::UnknownInstruction { name } => self
                    .msgr
                    .error(format!("Unknown instruction {}", name.purple()).as_str()),
            },
        }
    }
}
