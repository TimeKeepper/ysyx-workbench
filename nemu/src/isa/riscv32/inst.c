...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, Print_DBG_Message("srl"),R(rd) = src1 >> src2);
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, Print_DBG_Message("sra"),R(rd) = (sword_t)src1 >> src2);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh    , R, Print_DBG_Message("mulh"),R(rd) = (word_t)(((int64_t)src1 * (int64_t)src2) >> 32));
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if (Print_DBG_Message("blt"),(sword_t)src1 < (sword_t)src2) s->dnpc += imm - 4);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if (Print_DBG_Message("bge"),(sword_t)src1 >= (sword_t)src2) s->dnpc += imm - 4);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if (Print_DBG_Message("bltu"),src1 < src2) s->dnpc += imm - 4);
