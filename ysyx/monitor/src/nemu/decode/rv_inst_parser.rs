    ...
    fn parse(&self, input: u32) -> Result<ExecuteInst, Option<()>> {
        if input & self.parser.0 == self.parser.1 {
            return Ok(ExecuteInst::new(
                &self.name,
                RiscvInst::extract_bits(input, 15..20) as u8,
                RiscvInst::extract_bits(input, 20..25) as u8,
                RiscvInst::extract_bits(input, 7..12) as u8,
                self.get_imm(input),
            ));
        }
        Err(None)
...
impl RvInstParser {
    ...
    pub fn new() -> Self {
        let rv32_i_inst = vec![
            ...
            RiscvInst::new("jal",       ImmType::U, "??????? ????? ????? ??? ????? 11011 11", vec!["j"]),
            RiscvInst::new("jalr",      ImmType::U, "??????? ????? ????? ??? ????? 11001 11", vec!["jr", "ret"]),
            ...
            RiscvInst::new("lb",        ImmType::U, "??????? ????? ????? 000 ????? 00000 11", vec![]),
            RiscvInst::new("lh",        ImmType::U, "??????? ????? ????? 001 ????? 00000 11", vec![]),
            RiscvInst::new("lw",        ImmType::U, "??????? ????? ????? 010 ????? 00000 11", vec![]),
            RiscvInst::new("lbu",       ImmType::U, "??????? ????? ????? 100 ????? 00000 11", vec![]),
            RiscvInst::new("lhu",       ImmType::U, "??????? ????? ????? 101 ????? 00000 11", vec![]),
            
            RiscvInst::new("sb",        ImmType::U, "??????? ????? ????? 000 ????? 01000 11", vec![]),
            RiscvInst::new("sh",        ImmType::U, "??????? ????? ????? 001 ????? 01000 11", vec![]),
            RiscvInst::new("sw",        ImmType::U, "??????? ????? ????? 010 ????? 01000 11", vec![]),

            RiscvInst::new("addi",      ImmType::U, "??????? ????? ????? 000 ????? 00100 11", vec!["nop", "li", "mv"]),
