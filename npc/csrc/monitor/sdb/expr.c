...
static bool make_token(char *e) {
    ...
    /* Try all rules one by one. */
        for (i = 0; i < NR_REGEX; i ++) {
            if (regexec(&re[i], e + position, 1, &pmatch, 0) != 0 || pmatch.rm_so != 0) continue;
            ...
            switch (rules[i].token_type) {
                ...
                default: printf("You have type some unknown token!\n"); return false;
            }
            break;
        }
        if (i == NR_REGEX) {
...
static int find_Op(int p, int q){
    ...
    for(i = p; i <= q; i++){  
        switch (tokens[i].type){
            case TK_LPAREN: op++; break;
            case TK_RPAREN: op--; break;
            case TK_DEREF:
                if(op == 0 && (op_type == 0)){
                    op_type = i;
                }
    ...
    if(*success == false) return 0;
        ...
        *success = false;
        ...
        if(strcmp(tokens[p].str, "$pc\0") == 0) return cpu.pc;
            return reg_name2id(tokens[p].str+1);
        }
        else{
            *success = false;
            return 0;
        ...
        switch(tokens[index].type){
            ...
            default:        printf("Unknown condition, you should check you code again!\n"); assert(0);
        }
    }
}
