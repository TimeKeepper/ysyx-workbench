                ...
                for(uint32_t i = 0; i < CONFIG_ICache_Set; ++i) {
                    ...
                    for(uint32_t j = 0; j < CONFIG_ICache_Way; ++j) {
                        std::cout << ANSI_FG_CYAN"valid " << (this->emulator->cache[i][j].valid ? ANSI_FG_GREEN"true" : ANSI_FG_RED"false") << '\t'
                            << ANSI_FG_CYAN"tag[" << ICACHE_TAG_BITS << "] " <<  std::hex << ANSI_FG_BLUE"0x" << this->emulator->cache[i][j].tag << '\t'
                            << ANSI_FG_CYAN"data " << ANSI_FG_BLUE << "0x" << std::setw(8) << std::setfill('0') << this->emulator->cache[i][j].inst << std::dec << '\t'
                            << ANSI_NONE << std::endl;
                    }
                    std::cout << std::endl;
                }
