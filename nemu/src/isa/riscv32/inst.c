...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, Print_DBG_Message("srai"),R(rd) = src1 >> imm);
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , I, Print_DBG_Message("sll"),R(rd) = src1 << src2);
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, imm^=0x00000fe00,Print_DBG_Message("sb"),Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, imm^=0x00000fe00,Print_DBG_Message("sh"),Mw(src1 + imm, 2, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, imm^=0x00000fe00,Print_DBG_Message("sw"),Mw(src1 + imm, 4, src2));
