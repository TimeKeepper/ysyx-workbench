...
class ysyx_23060198_WBU extends Module {
    val io = IO(new Bundle{
        ...
        val WBU_2_REG = Decoupled(Output(new BUS_WBU_2_REG))
    })
    ...
    state := MuxLookup(state, s_wait_valid)(
        Seq(
            ...
            s_wait_ready -> Mux(io.WBU_2_REG.ready, s_wait_valid, s_wait_ready),
        )
    )
    ...
    io.WBU_2_REG.bits.GPR_wen <> io.EXU_2_WBU.bits.RegWr

    io.WBU_2_REG.bits.CSR_ctr <> io.EXU_2_WBU.bits.csr_ctr

    io.WBU_2_REG.bits.CSR_waddra := MuxLookup(io.EXU_2_WBU.bits.csr_ctr, io.EXU_2_WBU.bits.Imm(11, 0))(Seq(
        CSR_R1W2 -> "h341".U
    ))

    io.WBU_2_REG.bits.CSR_waddrb := "h342".U

    io.WBU_2_REG.bits.CSR_wdataa := MuxLookup(io.EXU_2_WBU.bits.csr_ctr, io.EXU_2_WBU.bits.Result)(Seq(
        CSR_R1W2 -> io.EXU_2_WBU.bits.PC,
    ))

    io.WBU_2_REG.bits.CSR_wdatab := 11.U
}